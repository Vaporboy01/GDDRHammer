...
main (int argc, char *argv[])
{
  ...
  fprintf(stderr, "%sSize: %lu, Range: %lu, Iterations: %lu, Step: %lu, Threshold: %lu, Max Banks: %lu, Output File: %s\n",
          CLI_PREFIX.c_str(), size, range, it, step, threshold, max_bank,
          banks_filename);
  ...
  nc_test.loop_range (
      [&nc_test, &bank_map, threshold] (uint64_t step) {
        ...
        // fprintf(stderr, "Addr: %lu, base_delay: %lu\n", step, base_delay);
  ...
  for (auto &it : bank_map){
    fprintf(stderr, "Bank start addr: %lu\n", it.first);
